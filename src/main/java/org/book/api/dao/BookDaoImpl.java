...
public class BookDaoImpl implements BookDao{
    ...
    public Book addBook(Book book) {
        ...
        String checkSql = "select count(*) from book where bookId=?";
        int count = jdbcTemplate.queryForObject(checkSql, args, Integer.class);
        if (count > 0)
    ...
    public Book updateBook(int bookId, Book book) {
        String sql = "update book set title = ?, author = ?, publications = ?, price = ?, subject = ?  where bookId = ?";
        Object[] args = {book.getAuthor(), book.getPublications(), book.getPrice(), book.getSubject(), bookId};
        int rows = jdbcTemplate.update(sql, args);
        return rows > 0 ? book : null;
    }
